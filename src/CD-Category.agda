...
import Shapes
module cdcatshp = Shapes cat
open cdcatshp
open cdcatshp.CommutativeSquare
...
record CD-Category : (Set (n ⊔ m)) where
  ...
  π₁' : {!!}
  π₁' = let l = forward idIso
            m = θ ε -- nat trans between monoidal functors?
            mm = forward rightUnitor
            -- horizontal composition of nat transforms?
            r = {!? 𝕏ₙ ?!} ●ₕ (forward rightUnitor)
        in {!!} -- (idIso ⊗≅ (θ ε)) ●̬  (forward rightUnitor)
